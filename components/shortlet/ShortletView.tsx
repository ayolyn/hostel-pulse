...
import { Home, Loader2, Search, SlidersHorizontal, ChevronDown, X, ArrowLeft } from 'lucide-react';
...
export function ShortletView({ isEmbedded = false }: { isEmbedded?: boolean }) {
    ...
    const clearFilters = () => {
        if (isEmbedded) {
            router.push('/dashboard/student?tab=shortlet');
        } else {
    ...
    return (
        <div className={isEmbedded ? "animate-in fade-in slide-in-from-bottom-4 duration-700" : ""}>
            ...
            <PropertyFilterBar mode="shortlet" basePath={isEmbedded ? "/dashboard/student" : "/shortlet"} />
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-32 px-4 text-center bg-white dark:bg-neutral-900 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm">
                    ...
                    <h3 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">No shortlets available here yet.</h3>
