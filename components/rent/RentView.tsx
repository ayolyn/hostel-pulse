...
export function RentView({ isEmbedded = false }: { isEmbedded?: boolean }) {
    ...
    const clearFilters = () => {
        if (isEmbedded) {
            router.push('/dashboard/student?tab=find-hostel');
        } else {
            router.push('/rent');
        }
    };
    ...
    return (
        <div className={isEmbedded ? "animate-in fade-in slide-in-from-bottom-4 duration-700" : ""}>
            ...
            <PropertyFilterBar mode="rent" basePath={isEmbedded ? "/dashboard/student" : "/rent"} />
