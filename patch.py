...
file_path = 'components/dashboard/ListingStudio.tsx'

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Add video_url to form state
content = content.replace("is_newly_built: false,", "is_newly_built: false,\n        video_url: '',")
...
video_upload_code = """
    const handleVideoUpload = async (file: File) => {
        ...
        const { data, error } = await supabase.storage
            .from('market-images') // reuse existing public bucket or change if you have a video bucket
...
"""
content = content.replace("const removeImage = (url: string) => {", video_upload_code + "\n    const removeImage = (url: string) => {")
...
# Reset video_url on success
content = content.replace("is_newly_built: false, youtube_video_url: '',", "is_newly_built: false, video_url: '', youtube_video_url: '',")
...
"""
content = content.replace("                        <label className=\"text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-neutral-500 px-2\">Media Links</label>", ui_code + "\n                        <label className=\"text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-neutral-500 px-2\">External Media Links</label>")

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Patch applied to ListingStudio.tsx")
