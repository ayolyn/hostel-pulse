...
function RentContent() {
    ...
    return (
        <>
            <div className="mb-12">
                <h1 className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tighter mb-2">Properties for Rent</h1>
                <p className="text-gray-500 font-medium">
                    {loading ? 'Refreshing results...' : `${properties.length} listing${properties.length !== 1 ? 's' : ''} available in Ogbomoso`}
                </p>
            </div>
                    ...
                    {properties.map(p => (
                        <Link key={p.id} href={`/property/${p.id}`} className="block group">
                            <PropertyCard
                                ...
                                rating={4.8}
                                image={p.images?.[0] ?? 'https://images.unsplash.com/photo-1555854877-bab0e564b8d5?q=80&w=600'}
