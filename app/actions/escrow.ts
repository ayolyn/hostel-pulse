...
export async function releaseEscrowFunds(transactionId: string) {
    ...
    if (transaction.payee_id) {
        ...
        if (agentData) {
            ...
            const { data: landlordData } = await db.from('landlord_accounts').select('deals_closed').eq('id', transaction.payee_id).maybeSingle();
            if (landlordData) {
                await db.from('landlord_accounts').update({ deals_closed: Number((landlordData as any).deals_closed || 0) + 1 }).eq('id', transaction.payee_id);
            }
        }
    }
